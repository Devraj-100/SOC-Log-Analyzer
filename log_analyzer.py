...
Features: brute force, error spikes, SQLi, path traversal, anomaly scoring,
structured JSON reporting, SIEM-ready output, multi-log merging.
"""
...
CONFIG = {
    ...
    "report_file": None,                 # None = stdout, path = file
    "json_report": "log_analysis_report.json",  # Dedicated JSON report path
    "merge_existing": False,             # Merge with existing JSON report
...
SOC_REPORT_TEMPLATE = {
    ...
    "statistics": {
        ...
        "total_successful_auths": 0,
        "total_http_errors": 0,
        "total_sqli_attempts": 0,
        "total_xss_attempts": 0,
        "total_path_traversals": 0,
        "distinct_ips_total": 0,
        "severity_breakdown": {
...
class LogAnalyzer:
    def __init__(self, config=None):
        ...
        self.sqli_attempts = defaultdict(lambda: defaultdict(list))  # ip -> {type: [paths]}
    ...
    def generate_report(self):
        ...
        if report["statistics"]["total_failed_auths"] > 0:
            recs.append("Implement account lockout policy after 5 failed attempts (e.g., `faillock` or `pam_tally2`).")
        if any(f["severity"] in ("CRITICAL", "HIGH") for f in report["findings"]):
            recs.append("Block identified attacking IPs at the firewall (iptables / nftables / cloud WAF).")
        if report["statistics"]["total_sqli_attempts"] > 0:
            recs.append("Enforce prepared statements / parameterized queries for all database interactions.")
            recs.append("Deploy a WAF (ModSecurity / Cloudflare / AWS WAF) with SQLi rulesets.")
        ...
        if report["statistics"]["total_http_errors"] > 100:
            recs.append("Review error pages to ensure they do not leak stack traces or paths.")
        if report["findings"]:
    ...
    def _save_json_report(self, report):
        """Save the structured JSON report to disk."""
        ...
        # If merging with existing report
        if merge and path.exists():
            try:
                with open(path, "r") as f:
                    existing = json.load(f)
                # Merge findings (deduplicate by IP)
                ...
                existing["statistics"]["severity_breakdown"] = report["statistics"]["severity_breakdown"]
                report = existing
                print(f"[+] Merged with existing report: {json_path}")
            except (json.JSONDecodeError, KeyError) as e:
                print(f"[!] Could not merge with existing report ({e}), overwriting.")

        with open(path, "w") as f:
            json.dump(report, f, indent=2)

        print(f"[✓] JSON report saved: {json_path} ({len(report['findings'])} findings)")
        print(f"    Severity breakdown: {json.dumps(report['statistics']['severity_breakdown'])}")

        # Also print the report in plain text if no output format set otherwise
        if self.config["output_format"] == "json":
            print(json.dumps(report, indent=2))

    # ─── Plain Text / CSV Output (kept for CLI convenience) ───────────────

    def print_plain_report(self, report):
        """Fallback plain text output."""
        lines = [
            "=" * 75,
            "  🛡️  LOG ANALYZER — SOC REPORT",
            f"  Generated: {report['report_metadata']['generated_at']}",
            f"  Tool: {report['report_metadata']['tool']} v{report['report_metadata']['version']}",
            f"  Analyst: {report['report_metadata']['analyst']}",
            "=" * 75,
            "",
            "  📊 SCAN SUMMARY",
            ...
            f"    Total distinct IPs : {report['statistics']['distinct_ips_total']}",
            "",
            f"  🎯 SEVERITY BREAKDOWN",
        ]
        for sev in ["CRITICAL", "HIGH", "MEDIUM", "LOW", "INFO"]:
            count = report['statistics']['severity_breakdown'].get(sev, 0)
        ...
        if not report["findings"]:
            ...
            lines.append(f"  {'IP':<18} {'Score':<7} {'Severity':<10} {'AuthFail':<9} "
                         f"{'HTTPErr':<8} {'SQLi':<5} {'XSS':<4} {'Traverse':<9} {'Threats'}")
            lines.append("-" * 90)
            for f in report["findings"]:
                ...
                lines.append(
                    ...
                    f"{f['failed_auths']:<9} {f['http_errors']:<8} "
                    f"{sum(f['sqli_attempts'].values()):<5} {f['xss_attempts']:<4} "
                    f"{f['path_traversals']:<9} {threats}"
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Log Analyzer — SOC Analyst Edition 🛡️  Detect suspicious activity in system/web logs",
...
Examples:
  ...
  # Web server logs with custom threshold
  python log_analyzer.py /var/log/nginx/access.log -t 10 -w 15

  # Save structured JSON report (default)
  python log_analyzer.py /var/log/auth.log -jr my_report.json

  # Merge with an existing report (accumulate findings over time)
  python log_analyzer.py /var/log/auth.log -jr weekly_report.json --merge

  # CSV for spreadsheet import
  python log_analyzer.py /var/log/auth.log -f csv -o report.csv

  # Analyze multiple sources at once
  python log_analyzer.py /var/log/auth.log /var/log/nginx/access.log /var/log/syslog
        """
    )
    ...
    parser.add_argument("--merge", action="store_true",
                        help="Merge findings with existing JSON report instead of overwriting")
    ...
    # Print plain version for convenience
    if args.format != "json":
